...
function commandParserFactory(Logger, Promise, _) {
    ...
    CommandParser.prototype[ls] = function(data) {
        ...
        try {
             var allfiles = data.stdout.split('\n');
             var lines = allfiles.slice(1,-1);
             var parsed = [];
              _.forEach(lines, function(line){
                if(!line.startsWith('total')){
                    var file = {};
                    var split = line.split(" ").filter(Boolean);
                    console.log("kelly====================split" + split);
                    var permission = {};
                    //var version i= split.splice(11,1);
                    permission['user'] = split[0].substr(1,4);
                    permission['group'] = split[0].substr(4,7);
                    permission['other'] = split[0].substr(7,10);
                    file['permision'] = permission;
                    file['link_count']= split[1];
                    file['owner']= split[2];
                    file['group']= split[3];
                    file['size']= split[4];
                    file['modified_time']= split[5] +" "+ split[6] + " " + split[7];
                    file['name']= split[8];
                    ...
                    console.log("kelly===================file" + file);
                    parsed.push(file);
                }
            });
                    console.log("kelly====================parsed" + parsed);
            return Promise.resolve({ data: parsed, source: 'ls', store: true });
        } catch (e) {
                    console.log("kelly====================err" + e);
            return Promise.resolve({ source: 'ls', error: e });
        }
    };
